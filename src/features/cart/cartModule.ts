...
const removeItem = (id: number): void => {
  ...
  if (isExist.quantity > 1) {
    isExist.quantity--;
    console.log("item quantity decreased !", isExist);
...
const addDiscount = (discountCode: string): void => {
  ...
  if (checkCode) {
    if (checkCode.type === "cart") {
      ...
      const product =
        cart.items[
          cart.items.findIndex((item) => item.id === checkCode.itemId)
        ];
...
const getCartItemPrice = (item: CartItem): number => {
  ...
  if (codeApplied) {
    ...
    totalPrice =
      ...
      (item.price * item.quantity * totalDiscount) / 100;
  } else {
...
const getTotalPrice = (): number => {
  ...
  const haveDiscount = cart.items.filter(
    (item) => item.discountApplied.length > 0
  );
  ...
  if (haveDiscount) {
    ...
    totalDiscountedItemPrice = discountedItemPrice.reduce(
...
const getCartItems = (): CartItem[] => {
  const cartItems = cart.items.filter(
    (product, index) =>
      product.quantity >= 1 &&
      cart.items.findIndex((item) => item.id === product.id) === index
  );
  return cartItems;
};
